...
void UOverlayWidgetController::BindCallbacksToDependencies()
{
	...
	if (UAuraAbilitySystemComponent* AuraASC = Cast<UAuraAbilitySystemComponent>(AbilitySystemComponent))
	{
		...
		AuraASC->EffectAssetsTagDelegate.AddLambda(
...
void UOverlayWidgetController::OnXPChanged(int32 NewXP) const
{
	...
	if (Level <= MaxLevel && Level > 0)
	{
		...
		const int32 CurXPForThisLevel = NewXP - PreviousLevelUpRequirement;

		const float XPBarPercent = static_cast<float>(CurXPForThisLevel) / static_cast<float>(DeltaLevelRequirement);
