...
void UAuraAttributeSet::HandleIncomingXP(FEffectProperties Props)
{
	...
	{
		const int32 CurLevel = ICombatInterface::Execute_GetLevel(Props.SourceCharacter);
		...
		if (NumLevelUps > 0)
		{
			...
			for (int i = 0; i < NumLevelUps; ++i)
			{
				AttributePointsReward +=
					IPlayerInterface::Execute_GetAttributePointsReward(Props.SourceCharacter, CurLevel + i);
				SpellPointsReward +=
					IPlayerInterface::Execute_GetSpellPointsReward(Props.SourceCharacter, CurLevel + i);
			}
...
void UAuraAttributeSet::Debuff(const FEffectProperties& Props)
{
	...
	const FGameplayTag DebuffTag = DamageToDebuff[DamageType];
